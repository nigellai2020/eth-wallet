...
(function (Contract) {
    class TContract {
        ...
        methodsFromWeiArray(...args) {
            ...
            return new Promise(async function (resolve, reject) {
                ...
                var arr = [];
                for (var i = 0; i < result.length; i++) {
                    arr.push(new BigNumber.default(self.wallet.utils.fromWei(result[i])));
                }
                resolve(arr);
            });
        }
        methodsFromWei(...args) {
            ...
            return new Promise(async function (resolve, reject) {
                let result = await self.methods.apply(self, args);
                return resolve(new BigNumber.default(self.wallet.utils.fromWei(result)));
            });
        }
        methods(...args) {
    ...
    class TAuthContract extends TContract {
        rely(address) {
    ...
    Contract.TAuthContract = TAuthContract;
})(Contract || (Contract = {}));
