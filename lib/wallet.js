    ...
    async _send(abiHash, address, methodName, params, options) {
        ...
        try {
            if (!methodName) {
                ...
                let signer = await this.getSigner();
                const ethersReceipt = await signer.sendTransaction(Object.assign(Object.assign({}, tx), { gasPrice: tx.gasPrice instanceof bignumber_js_1.BigNumber ? tx.gasPrice.toFixed() : tx.gasPrice }));
                receipt = {
                    status: BigInt(1),
                    transactionHash: ethersReceipt.hash,
