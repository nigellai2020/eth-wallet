...
(function (Wallet_1) {
            ...
            return new Promise(async function (resolve, reject) {
                try {
                    ...
                    try {
                        ...
                        if (error.code === 4902) {
                            try {
                                let network = Wallet_1.DefaultNetworks[chainId];
                                if (!network)
                                    resolve(false);
    ...
    class Wallet {
        ...
        setDefaultProvider() {
            if (!this.chainId)
                this.chainId = 56;
            if (Wallet_1.DefaultNetworks[this.chainId] && Wallet_1.DefaultNetworks[this.chainId].rpcUrls.length > 0) {
                this.provider = Wallet_1.DefaultNetworks[this.chainId].rpcUrls[0];
            }
        }
        ...
        get balance() {
            ...
            return new Promise(async function (resolve) {
                try {
                    let network = Wallet_1.DefaultNetworks[self.chainId];
                    let decimals = 18;
