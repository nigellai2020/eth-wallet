    ...
    initEvents() {
        ...
        if (this.installed()) {
            this.provider.on('accountsChanged', (accounts) => {
                ...
                eventBus_1.EventBus.getInstance().dispatch(constants_1.ClientWalletEvent.AccountsChanged, accountAddress);
...
class Web3ModalProvider extends EthereumProvider {
    ...
    async connect() {
        ...
        this._provider = await Web3Modal.init(Object.assign({ showQrModal: true, qrModalOptions: { themeMode: "light" }, methods: ["eth_sendTransaction", "personal_sign"], events: ["chainChanged", "accountsChanged"] }, this.web3ModalOptions));
...
class Wallet {
    ...
    registerWalletEvent(sender, event, callback) {
        return eventBus_1.EventBus.getInstance().register(sender, event, callback);
    }
    ;
    ...
    async disconnect() {
        if (this.clientSideProvider) {
            ...
            eventBus_1.EventBus.getInstance().dispatch(constants_1.ClientWalletEvent.AccountsChanged, null);
        }
        this.setDefaultProvider();
    }
    get accounts() {
    ...
    getGasPrice() {
        return new Promise(async (resolve, reject) => {
            await this.init();
            try {
                resolve(new bignumber_js_1.BigNumber(await this._web3.eth.getGasPrice()));
            }
            catch (e) {
                reject(e);
            }
...
class RpcWallet extends Wallet {
    ...
    registerWalletEvent(sender, event, callback) {
        ...
        if (event == constants_1.RpcWalletEvent.Connected) {
            const accountsChangedRegistry = eventBus.register(sender, constants_1.ClientWalletEvent.AccountsChanged, (account) => {
                eventBus.dispatch(eventId, this.isConnected);
            });
            const chainChangedRegistry = eventBus.register(sender, constants_1.ClientWalletEvent.ChainChanged, (chainIdHex) => {
                eventBus.dispatch(eventId, this.isConnected);
            });
