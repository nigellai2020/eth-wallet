    ...
    initEvents() {
        ...
        if (this.installed()) {
            this.provider.on('accountsChanged', (accounts) => {
                ...
                eventBus_1.EventBus.getInstance().dispatch('accountsChanged', accountAddress);
            ...
            this.provider.on('chainChanged', (chainId) => {
                ...
                eventBus_1.EventBus.getInstance().dispatch('chainChanged', chainId);
                if (self.onChainChanged)
                    self.onChainChanged(chainId);
            });
            this.provider.on('connect', (connectInfo) => {
                eventBus_1.EventBus.getInstance().dispatch('connect', connectInfo);
                if (self.onConnect)
                    self.onConnect(connectInfo);
            });
            this.provider.on('disconnect', (error) => {
                eventBus_1.EventBus.getInstance().dispatch('disconnect', error);
...
class Wallet {
    ...
    registerClientWalletEvent(sender, event, callback) {
        return eventBus_1.EventBus.getInstance().register(sender, event, callback);
    }
    registerRpcWalletEvent(sender, instanceId, event, callback) {
        const eventId = instanceId + event;
        return eventBus_1.EventBus.getInstance().register(sender, eventId, callback);
    }
