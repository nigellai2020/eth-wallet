...
class Wallet {
    ...
    async _txObj(abiHash, address, methodName, params, options) {
        ...
        if (options && (options.gas || options.gasLimit)) {
            ...
            try {
                let gas = tx.gas = Number(await method.estimateGas({ from: this.address, to: address ? address : undefined, value: tx.value }));
...
class RpcWallet extends Wallet {
    ...
    registerWalletEvent(sender, event, callback) {
        ...
        if (event == constants_1.RpcWalletEvent.Connected) {
            const accountsChangedRegistry = eventBus.register(sender, constants_1.ClientWalletEvent.AccountsChanged, (payload) => {
