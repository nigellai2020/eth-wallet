...
exports.signTypedDataWithPrivateKey = void 0;
...
function encodeField(types, name, type, value, version) {
    if (types[type] !== undefined) {
        return [
            ...
            version === types_1.SignTypedDataVersion.V4 && value == null
                ...
                : arrToBufArr(keccak_1.keccak256(encodeData(type, value, types, version))),
        ];
    }
    if (value === undefined) {
    ...
    if (type === 'bytes') {
        ...
        return ['bytes32', arrToBufArr(keccak_1.keccak256(value))];
    }
    if (type === 'string') {
        if (typeof value === 'number') {
            value = numberToBuffer(value);
    ...
    if (type.lastIndexOf(']') === type.length - 1) {
        ...
        return [
            'bytes32',
            arrToBufArr(keccak_1.keccak256(rawEncode(typeValuePairs.map(([t]) => t), typeValuePairs.map(([, v]) => v)))),
        ];
    }
    return [type, value];
}
...
function eip712Hash(typedData, version) {
    ...
    return arrToBufArr(keccak_1.keccak256(Buffer.concat(parts)));
}
...
function signTypedDataWithPrivateKey({ privateKey, data, version, }) {
    ...
    const sig = ethereumjs_util_1.ecsign(messageHash, bufferPrivateKey);
